...
# send your archlinux related bug reports to amadren@protonmail.ch
###
...
printf '%s %s' "$(l10n 'build_pkg')" "$(printf '%s' "${desc}" | head -n1)"
if [ -n "${extra_infos}" ]; then
	printf ' (%s)' "${extra_infos}"
fi
printf '…\n'
if [ "${opts_quiet}" = '0' ]; then
	print wait
fi
mkdir -p "${PKG_TMPDIR}"
TMPDIR="${PKG_TMPDIR}" fakeroot -- dpkg-deb -Z"${compression}" -b "${dir}" "${PWD}/${dir##*/}.deb" 1>/dev/null
...
build_pkg_arch(){
	...
	local desc="$2"
	local vers="$3"
	local s_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
	printf '%s %s' "$(l10n 'build_pkg')" "$(printf '%s' "${desc}" | head -n1)"
	if [ -n "${extra_infos}" ]; then
		printf ' (%s)' "${extra_infos}"
	fi
...
for dep in $@; do
	case $dep in
		...
		*)
			if [ -z $(which "${dep}") ]; then
				print error
				printf '%s %s.\n%s.\n\n' "${dep}" "$(l10n 'not_found')" "$(l10n 'check_deps_error')"
				exit 1
			fi
		;;
	esac
...
for dep in $@; do
	if [ -z $(which "${dep}") ]; then
		...
		case $dep in
			icotool|wrestool)
				if [ "${NO_ICON}" = '0' ]; then
					printf '%s.\n%s.\n' "$(l10n 'check_deps_icons_1_icoutils')" "$(l10n 'check_deps_icons_2')"
					check_deps_soft_userinput_icons
				fi
			;;
			convert)
				if [ "${NO_ICON}" = '0' ]; then
					printf '%s.\n%s.\n' "$(l10n 'check_deps_icons_1_imagemagick')" "$(l10n 'check_deps_icons_2')"
					check_deps_soft_userinput_icons
				fi
			;;
		esac
...
if [ -n "$(which 7zr)" ]; then
	...
	local archive="$1"
	local target="$2"
	7zr x -o"${target}" -y "${archive}" >/dev/null
	}
elif [ -n "$(which 7za)" ]; then
	extract_7z() {
	local archive="$1"
	local target="$2"
	7za x -o"${target}" -y "${archive}" >/dev/null
	}
	...
	print error
	printf '%s:\np7zip, p7zip-full, unar\n%s.\n\n' "$(l10n 'pkgs_not_found_1')" "$(l10n 'pkgs_not_found_2')"
	exit 1
fi
}
